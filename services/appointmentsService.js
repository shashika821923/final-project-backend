...
exports.getAllAppointments = async (req, res) => {
  try {
    ...
    JOIN [lords-gym].[dbo].[users] AS U ON A.userId = U.userId where A.isApproved = 1 AND A.isDeleted = 0 AND A.isCompleted = 0 ORDER BY A.date, A.time`
        : req.body.status == '2'
        ? `SELECT A.appointmentId, A.userId, A.date, A.time, A.isDeleted, A.isApproved, A.isCompleted, U.firstName, U.secondName FROM [lords-gym].[dbo].[appointments] AS A
    JOIN [lords-gym].[dbo].[users] AS U ON A.userId = U.userId where A.isApproved = 0 and A.isDeleted = 0 and A.isCompleted = 0 ORDER BY A.date, A.time`
        : req.body.status == '3'
        ? `SELECT A.appointmentId, A.userId, A.date, A.time, A.isDeleted, A.isApproved, A.isCompleted, U.firstName, U.secondName FROM [lords-gym].[dbo].[appointments] AS A
    JOIN [lords-gym].[dbo].[users] AS U ON A.userId = U.userId where A.isApproved = 1 and A.isDeleted = 0 and A.isCompleted = 1 ORDER BY A.date, A.time`
        : `SELECT A.appointmentId, A.userId, A.date, A.time, A.isDeleted, A.isApproved, A.isCompleted, U.firstName, U.secondName FROM [lords-gym].[dbo].[appointments] AS A
    JOIN [lords-gym].[dbo].[users] AS U ON A.userId = U.userId where A.isDeleted = 1 ORDER BY A.date, A.time`;
...
exports.completeAppointment = async (req, res) => {
  try {
    ...
    console.error('Error Compelting appointment:', error.message);
    res.status(500).send('Error Compelting appointment');
  } finally {
    sql.close();
  }
}
